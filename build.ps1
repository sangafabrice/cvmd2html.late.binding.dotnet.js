...
& {
  ...
  & "$PSScriptRoot\TlbImp.exe" /nologo /silent 'C:\Program Files\Common Files\System\ado\msado60.tlb'  /out:$(($AdoDllPath = "$BinDir\Interop.ADODB.dll")) /namespace:ADODB
  ...
  jsc.exe /nologo /target:$($DebugPreference -eq 'Continue' ? 'exe':'winexe') /win32res:$ResFile /reference:$StdRegProvDll /reference:$WshDllPath /reference:$SWbemDllPath /reference:$MshtmlDllPath /reference:$AdoDllPath /out:$(($ConvertExe = "$BinDir\cvmd2html.exe")) $AssemblyInfoJS "$SrcDir\converter.js" "$SrcDir\msgbox.js" "$SrcDir\package.js" "$SrcDir\parameters.js" "$PSScriptRoot\index.js" "$SrcDir\setup.js" "$SrcDir\utils.js"
  $Env:Path = $EnvPath
