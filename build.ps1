...
& {
  ...
  jsc.exe /nologo /target:library /reference:$SWbemDllPath /out:$(($StdRegProvDll = "$BinDir\StdRegProv.dll")) /define:StdRegProvWim $AssemblyInfoJS "$SrcDir\StdRegProv.js"
  jsc.exe /nologo /target:library /reference:$SWbemDllPath /out:$(($Win32ProcessDll = "$BinDir\Win32.Process.dll")) /define:Win32ProcessWim $AssemblyInfoJS "$SrcDir\Win32.Process.js"
  jsc.exe /nologo /target:$($DebugPreference -eq 'Continue' ? 'exe':'winexe') /reference:$StdRegProvDll /reference:$Win32ProcessDll /reference:$WshDllPath /reference:$ShellDllPath /reference:$SWbemDllPath /out:$(($ConvertExe = "$BinDir\cvmd2html.exe")) "$SrcDir\AssemblyInfo.js" "$SrcDir\errorLog.js" "$SrcDir\package.js" "$SrcDir\parameters.js" "$PSScriptRoot\index.js" "$SrcDir\setup.js" "$SrcDir\utils.js"
  $Env:Path = $EnvPath
