...
function CreateConsoleHost(pwshExePath, pwshScriptPath) {
  ...
  ConsoleHost.StartWith = function(markdownPath) {
    WaitForExit(WshShell.Exec(format('"{0}" -nop -ep Bypass -w Hidden -cwa "try { Import-Module $args[0]; {3} -MarkdownPath $args[1] } catch { Write-Error $_.Exception.Message }" "{1}" "{2}"', [pwshExePath, pwshScriptPath, markdownPath, FileSystem.GetName(pwshScriptPath).replace(/\.[^\.]+$/i, '')])));
  }
